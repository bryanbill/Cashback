...
public class OffersAdapter
    extends RecyclerView.Adapter<OffersAdapter.OffersResultHolder> {
  private Context context;
  private List<OfferModel> results = new ArrayList<>();

  public OffersAdapter(Context context) { this.context = context; }

  private static final DecimalFormat df = new DecimalFormat("0.0");

  @NonNull
  @Override
  public OffersResultHolder onCreateViewHolder(@NonNull ViewGroup parent,
                                               int viewType) {
  ...
  @Override
  public void onBindViewHolder(@NonNull OffersResultHolder holder,
                               int position) {
    try {
      OfferModel offer = results.get(position);

      holder.productNameTextView.setText(offer.getProductName());
      holder.productDate.setText(
          DateUtil.getFormattedDate(offer.getOfferEndDate()));
      holder.productSize.setText(offer.getProductSize());
      holder.productCashback.setText(offer.getProductCashBack().toString());
      holder.productCommission.setText(
          df.format((offer.getProductCashBack() *
                     offer.getPromotionDetailsRetailerCommission())));
      Glide.with(context)
          ...
          //                        resource, Object model, Target<Drawable>
          //                        target, DataSource dataSource, boolean
          //                        isFirstResource) {
          //                            return false;
          //                        }
          //                    })
  ...
  public class OffersResultHolder extends RecyclerView.ViewHolder {
    private TextView productNameTextView, productDate, productSize,
        productCashback, productCommission;
